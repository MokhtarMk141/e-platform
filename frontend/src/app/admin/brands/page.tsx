...
export default function BrandsPage() {
  ...
  const handleDelete = async (id: string, name: string) => {
    if (window.confirm(`Are you sure you want to delete brand "${name}"? Products linked to it will lose their brand.`)) {
      try {
  ...
  const SortArrow = ({ col }: { col: typeof sortBy }) =>
    sortBy === col ? <span style={{ color: "var(--brand-red)", marginLeft: 4, fontSize: 11 }}>{sortDir === "asc" ? "▲" : "▼"}</span> : <span style={{ opacity: 0.25, marginLeft: 4, fontSize: 11 }}>▲</span>;
  ...
  return (
    <>
      <style>{`
        ...
        .search-input { width: 100%; padding: 9px 14px 9px 38px; background: var(--surface); border: 1px solid var(--border); border-radius: 10px; font-size: 13.5px; color: var(--foreground); font-family: 'Plus Jakarta Sans', sans-serif; transition: border-color 0.2s, box-shadow 0.2s; outline: none; box-sizing: border-box; }
        .search-input::placeholder { color: var(--text-dim); }
        .search-input:focus { border-color: var(--brand-red); box-shadow: 0 0 0 3px rgba(255,40,0,0.08); }
        .btn-add { display: flex; align-items: center; gap: 7px; padding: 9px 18px; background: var(--brand-red); color: #fff; border: none; border-radius: 10px; font-size: 13.5px; font-weight: 700; font-family: 'Plus Jakarta Sans', sans-serif; cursor: pointer; white-space: nowrap; transition: background 0.2s, transform 0.2s, box-shadow 0.2s; box-shadow: 0 4px 14px rgba(255,40,0,0.25); letter-spacing: -0.01em; }
        ...
        .brand-name { font-weight: 700; color: var(--foreground); font-size: 14px; letter-spacing: -0.01em; }
        .brand-desc { font-size: 12px; color: var(--text-dim); font-weight: 500; margin-top: 2px; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; }
        .brand-logo { width: 40px; height: 40px; border-radius: 10px; border: 1px solid var(--border); background: var(--background); object-fit: contain; padding: 6px; }
        ...
        .pg-btn { display: inline-flex; align-items: center; justify-content: center; width: 34px; height: 34px; border-radius: 8px; border: 1px solid var(--border); background: var(--background); color: var(--text-muted); cursor: pointer; transition: all 0.2s; font-size: 13px; font-weight: 600; }
      ...
      <div className="gap-page">
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16, flexWrap: "wrap" }}>
          <div>
            <h1 style={{ fontSize: 26, fontWeight: 800, letterSpacing: "-0.03em", margin: 0, color: "var(--foreground)" }}>Brands</h1>
            <p style={{ fontSize: 13, color: "var(--text-muted)", marginTop: 4, fontWeight: 500 }}>Manage your product brands</p>
          </div>
          <button className="btn-add" onClick={() => router.push("/admin/brands/add")}>
        ...
        <div style={{ display: "flex", gap: 14, marginTop: 24, flexWrap: "wrap" }}>
          <div className="stat-card">
            ...
            <p className="stat-sub">Available in catalog</p>
          </div>
        </div>
        ...
        <div className="toolbar">
          <div className="search-wrap">
            ...
            <input className="search-input" placeholder="Search brands..." value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} />
          </div>
          <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 6 }}>
            <Icon d={icons.sort} size={15} />
            <span style={{ fontSize: 13, color: "var(--text-muted)", fontWeight: 500 }}>{filtered.length} result{filtered.length !== 1 ? "s" : ""}</span>
          </div>
        </div>
        ...
        <div className="table-wrap">
          {loading ? (
            ...
            <div className="empty">
              ...
              <p className="empty-title">No brands found</p>
            ...
            <>
              <table>
                ...
                <tbody>
                  {paginated.map((brand: Brand, idx: number) => (
                    <tr key={brand.id}>
                      <td style={{ color: "var(--text-dim)", fontWeight: 600, fontSize: 12 }}>{(page - 1) * PAGE_SIZE + idx + 1}</td>
                      <td>{brand.logoUrl ? <img src={brand.logoUrl} alt={brand.name} className="brand-logo" /> : <div className="brand-logo" />}</td>
                      <td><div className="brand-name">{brand.name}</div></td>
                      <td><div className="brand-desc">{brand.description || "—"}</div></td>
                      <td style={{ textAlign: "right" }}><span style={{ fontWeight: 600, color: "var(--foreground)" }}>{brand.productCount || 0}</span></td>
                      <td>
                        <div style={{ display: "flex", gap: 6, justifyContent: "center" }}>
                          <button className="action-btn edit" title="Edit" onClick={() => router.push(`/admin/brands/edit/${brand.id}`)}>
                            <Icon d={icons.edit} size={14} />
                          </button>
                          <button className="action-btn del" title="Delete" onClick={() => handleDelete(brand.id, brand.name)}>
              ...
              <div className="pagination">
                <span className="pg-info">Showing {Math.min((page - 1) * PAGE_SIZE + 1, filtered.length)}-{Math.min(page * PAGE_SIZE, filtered.length)} of {filtered.length} brands</span>
                <div style={{ display: "flex", gap: 6 }}>
                  <button className="pg-btn" disabled={page === 1} onClick={() => setPage((p) => p - 1)}><Icon d={icons.chevronLeft} size={15} /></button>
                  {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
                    <button key={p} className={`pg-btn${p === page ? " active" : ""}`} onClick={() => setPage(p)}>{p}</button>
                  ))}
                  <button className="pg-btn" disabled={page === totalPages || totalPages === 0} onClick={() => setPage((p) => p + 1)}><Icon d={icons.chevronRight} size={15} /></button>
                </div>
              </div>
            </>
          )}
